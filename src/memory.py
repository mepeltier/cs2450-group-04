class Memory:
    ...
    def validate_address(self, address):
        ...
        if not (0 <= address < self.size):
            raise IndexError(f"Memory address {address} out of bounds. Valid range: 0-{self.size-1}")
    ...
    def load_program(self, program):
        ...
        if len(program) > self.size:
            raise ValueError(f"Program too large. Maximum size: {self.size} words")
            
        for i, instruction in enumerate(program):
            self.validate_word(instruction)
